  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-white">Trivia Database</h2>
        <button 
          onClick={openNewForm}
          className="flex items-center gap-2 bg-[#0ea5e9] hover:bg-[#0ea5e9]/90 text-white px-4 py-2 rounded-lg font-medium transition-colors"
        >
      ...
      {isFormOpen && (
        <div className="bg-gray-800/50 border border-gray-700 p-6 rounded-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:border-[#0ea5e9] focus:outline-none"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-400 mb-1">Question (English) *</label>
                <textarea
                  required
                  rows="2"
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Question (Sinhala)</label>
                <textarea
                  rows="2"
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-3">Options & Correct Answer</label>
              <div className="space-y-3">
                {formData.options.map((opt, index) => (
                  <div key={opt.id} className={`flex items-center gap-3 p-3 rounded-lg border ${formData.correctOption === opt.id ? 'border-[#0ea5e9] bg-[#0ea5e9]/5' : 'border-gray-700 bg-gray-900/50'}`}>
                    <input
                      type="radio"
                      name="correctOption"
                      checked={formData.correctOption === opt.id}
                      onChange={() => setFormData({...formData, correctOption: opt.id})}
                      className="w-5 h-5 text-[#0ea5e9] bg-gray-900 border-gray-600 focus:ring-[#0ea5e9]"
                    />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 flex-1">
                      <input
                        ...
                        className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-white text-sm focus:border-[#0ea5e9] focus:outline-none"
                      />
                      <input
                        type="text"
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-[#0ea5e9] text-white font-medium rounded-lg hover:bg-[#0ea5e9]/90 transition-colors"
              >
                Save Question
              </button>
            </div>
              ...
              questions.map((q) => (
                <tr key={q.id} className="hover:bg-gray-800/30 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="font-medium text-white">{q.questionEn}</div>
